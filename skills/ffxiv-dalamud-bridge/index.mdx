---
title: ffxiv-dalamud-bridge
description: Local bridge URL exposed by the Dalamud plugin. Defaults to http://127.0.0.1:47714.
---

# ffxiv-dalamud-bridge

This page mirrors the current skill definition from `/home/moodmnky/.openclaw/workspace/skills/ffxiv-dalamud-bridge/SKILL.md`.

## Registry fields

- Source: `/home/moodmnky/.openclaw/workspace/skills/ffxiv-dalamud-bridge/SKILL.md`
- Source group: Workspace Skills
- Machine-readable payload: `./skill.md`

The adjacent `skill.md` file is the canonical machine-readable skill artifact for MCP-based discovery and retrieval.
